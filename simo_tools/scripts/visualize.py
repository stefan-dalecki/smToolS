...
def visualize_table(
    ...
    handler.trio_lyze().identify_fractions().display_and_or_save(
...
def main(parser_args: argparse.Namespace):
    if not parser_args.display and not parser_args.save_images:
        raise ValueError(
            "This script is not worthwhile without specifying whether you want to"
            " display or save images."
        )
    logger.info("Beginning visualization.")
    parsers.ImportParser.validate(parser_args)
    save_location = (
        parsers.ExportParser.set_save_location(parser_args) or parser_args.save_location
    )
