...
) -> dict[str, Union[str, datetime.date]]:
    """
...
def reorder(df: pd.DataFrame, column_name: str, location: int) -> pd.DataFrame:
    ...
    if column_name not in df.columns:
        raise KeyError(
            f"Column '{column_name}' not found in dataframe columns: '{df.columns}'."
        )
...
class Script:
    ...
    def file_list(self) -> List[str]:
        ...
        if not self.directory and self.file:
            return [
                self.file
            ]  # just return file in list if we are not looking at a directory
    ...
    def _validate_brightness_method(self) -> None:
        if self.brightness_method:
            ...
            if self.filetype == cons.FileTypes.XML and self.brightness_method != "none":
                message = (
                    "Invalid brightness cutoff method for 'xml' filetype. Method must"
                    " be 'None'."
                )
            ...
            if self.brightness_method == "semi_auto" and not self.brightness_cutoffs:
                message = (
                    "Brightness cutoffs must be specified when using 'semi_auto' cutoff"
                    " method."
                )
                raise ValueError(message)
            _attribute_validation(
                self.brightness_method, self.brightness_method_options
            )
...
class FileReader:
    ...
    def one_table(self) -> pd.DataFrame:
        logger.info(
            f"Combining '{[file[0] for file in self.pre_processed_files]}' into one"
            " table."
        )
    ...
    def _process_csv(self) -> None:
        ...
        for file in self._rawfiles:
            ...
            if (
                cons.PRE_PROCESSED not in file
                and cons.Coordinates.set_of_options() <= set(track_df.columns)
            ):
            ...
            if not os.path.exists(h5_file_str):
                with ND2Reader(file) as movie:
                    ...
                    with tp.PandasHDFStoreBig(h5_file_str) as s:
                        ...
                        track_df = track_df[
                            track_df.groupby(cons.TRAJECTORY)[
                                cons.TRAJECTORY
                            ].transform("count")
                            > 1
                        ]
                        track_df = track_df.sort_values([
                            cons.TRAJECTORY, cons.FRAME
    ...
    def _process_h5(self) -> None:
        ...
        for file in self._rawfiles:
            with tp.PandasHDFStoreBig(file) as s:
                ...
                track_df = reorder(track_df, cons.FRAME, 1)
                track_df = track_df[
                    track_df.groupby(cons.TRAJECTORY)[cons.TRAJECTORY].transform(
                        "count"
                    )
                    > 1
                ]
    ...
    def _process_xml(self) -> None:
        ...
        for file in self._rawfiles:
            ...
            track_df = pd.DataFrame(
                columns=[cons.TRAJECTORY, *root[0][0].attrib.keys()]
            )
...
class Movie:
    ...
    def __init__(
        self, script_metadata: Script, filepath: str, trajectory_df: pd.DataFrame
    ) -> None:
        """
        ...
        self.name = {
            cons.FILENAME: os.path.splitext(os.path.basename(self.filepath))[0]
        }
        self._date = find_date(filepath)
        self._gasket = find_identifiers(
            filepath, os.sep, cons.GASKET, [cons.GASKET_ABV]
        )
