...
class Script:
    ...
    filetype_options = ["csv", "nd2", "h5", "xml"]
...
class FileReader:
    ...
    def process_csv(self) -> None:
        ...
        for file in self._rawfiles:
            ...
            if "pre_processed" not in file and {"x", "y"} <= set(track_df.columns):
            ...
            if not os.path.exists(h5_file_str):
                with ND2Reader(file) as movie:
                    ...
                    with tp.PandasHDFStoreBig(h5_file_str) as s:
                        ...
                        track_df = track_df[
                            track_df.groupby("Trajectory")["Trajectory"].transform(
                                "count"
                            )
                            > 1
                        ]
                        ...
                        track_df = fo.Form.reorder(track_df, "Trajectory", 0)
                        track_df = fo.Form.reorder(track_df, "Frame", 1)
    ...
    def process_h5(self) -> None:
        ...
        for file in self._rawfiles:
            with tp.PandasHDFStoreBig(file) as s:
                ...
                track_df = track_df.sort_values(["Trajectory", "Frame"]).reset_index(
                    drop=True
                )
                track_df = fo.Form.reorder(track_df, "Trajectory", 0)
                track_df = fo.Form.reorder(track_df, "Frame", 1)
                track_df = track_df[
                    track_df.groupby("Trajectory")["Trajectory"].transform("count") > 1
                ]
                self.pre_processed_files += [(file, track_df)]
