...
def trio_lyze(microscope: metadata.Microscope, df: pd.DataFrame) -> pd.DataFrame:
    ...
    if cons.BRIGHTNESS in df.columns:
        df[cons.AVERAGE_BRIGHTNESS] = df.groupby(cons.TRAJECTORY)[cons.BRIGHTNESS].transform(
            np.mean
        )
    df[cons.LENGTH_W_UNITS] = df.groupby(cons.TRAJECTORY)[cons.TRAJECTORY].transform("size")
