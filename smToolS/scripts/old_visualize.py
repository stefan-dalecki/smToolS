...
class Analyze:
    ...
    def trio_lyze(self):
        ...
        self.df["Length (frames)"] = self.df.groupby("Trajectory")["Trajectory"].transform("size")
        data = self.df.groupby("Trajectory")[["x", "y"]].apply(fo.Calc.one_step_MSD, self._metadata)
