    ...
    def convert(self) -> Self:
        ...
        if self.kinetic.name == "BSL":
            ...
            if self.components == 1:
                ...
                self.converted_popt[self.components :] = (
                    1e8 * self.popt[self.components :] ** 2 / (2 * self.microscope.framestep_size)
                )
                self.converted_pcov[self.components :] = (
                    1e8 * self.pcov[self.components :] ** 2 / (2 * self.microscope.framestep_size)
                )
        return self
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            table (pd.DataFrame): formatted data for fitting
        """
        assert components in RayDiff.component_options, "Not a valid component description"
        self.microscope = microscope
        self.movie = movie
        self._components = components
        self.kinetic = kinetic
