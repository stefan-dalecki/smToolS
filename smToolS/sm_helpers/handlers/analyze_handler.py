...
class AnalyzeHandler:
    ...
    def _apply_brightness(self):
        if self._script.brightness_method == "none" or self._script.filetype == cons.FileTypes.XML:
    ...
    def _apply_length(self):
        minimum_length = cut.Length(self._script.min_length, self._movie.data_df, method="minimum")
    ...
    def construct_kinetics(self) -> Self:
        ...
        self._rayd = (
            kin.Director(kin.RayD(self._microscope, self._movie.data_df[cons.SDS].dropna()))
            .construct_kinetic()
            .get_kinetic()
        )
    ...
    def display_and_or_save(self, save_location: str):
        if self._script.display or self._script.save_images:
            for model in self._models:
                ...
                figure.plot(self._script.display, self._script.save_images, save_location)
