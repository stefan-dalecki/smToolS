...
class Histogram:
    ...
    def lines(list, bins, low_cut, high_cut, title=None, x_label=None, y_label=None):
        """
...
class ExpDecay:
    ...
    def twocomp(
        ...
        plt.plot(
            x_data,
            cf.TwoCompExpDecay.equation(x_data, a, tau1, tau2),
            label=f"Maj Frac: {a1}\n"
    ...
    def explin(
        ...
        val2 = np.format_float_scientific(
            b * movie.framestep_size, precision=1, exp_digits=2
        )
        var1 = np.format_float_scientific(cov1, precision=1, exp_digits=2)
        var2 = np.format_float_scientific(cov2, precision=1, exp_digits=2)
        r_val = round(r2, 6)
        ...
        plt.plot(
            x_data,
            cf.ExpLinDecay.equation(x_data, tau1, b),
...
class Rayleigh:
    def onecomp(
        movie, df, a, sig, sig_cov, r2, kinetic, title=None, x_label=None, y_label=None
    ):
        x_data = df.iloc[:, 0].values
        y_data = df.iloc[:, 1].values
        raydiffcoeff = round(1e8 * sig**2 / (2 * movie.framestep_size), 4)
        varsig = np.format_float_scientific(sig_cov, precision=1, exp_digits=2)
        r_val = round(r2, 6)
        plt.scatter(
            x_data,
            y_data,
        ...
        plt.plot(
            x_data,
            cf.OneCompRayleigh.equation(x_data, a, sig),
    ...
    def twocomp(
        ...
        varsig1 = np.format_float_scientific(sig1_cov, precision=1, exp_digits=2)
        varsig2 = np.format_float_scientific(sig2_cov, precision=1, exp_digits=2)
        r_val = round(r2, 6)
        plt.scatter(
            x_data,
            y_data,
        ...
        plt.plot(
            x_data,
            cf.TwoCompRayleigh.equation(x_data, a, b, sig1, sig2),
